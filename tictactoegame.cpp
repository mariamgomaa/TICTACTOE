...
std::pair<int, int> TicTacToeGame::getAIMove() {
    ...
    case 1: return easyAI();
    case 2: return mediumAI();
    case 3: return getBestMove(AI);
    default: return easyAI();
    }
}
...
int TicTacToeGame::minimax(int depth, bool isMaximizing, int alpha, int beta, Player aiPlayer, Player humanPlayer) {
    ...
    if (isMaximizing) {
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                if (board[i][j] == NONE) {
                    board[i][j] = aiPlayer;
                    int eval = minimax(depth + 1, false, alpha, beta, aiPlayer, humanPlayer);
                    board[i][j] = NONE;
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                if (board[i][j] == NONE) {
                    ...
                    int eval = minimax(depth + 1, true, alpha, beta, aiPlayer, humanPlayer);
                    board[i][j] = NONE;
