...
private:
    ...
    // Helper methods
    void createTestUser(const QString &username, const QString &password, const QString &email = "");
    GameRecord createTestGameRecord(const QString &opponent, const QString &result);
...
void TestUserManager::createTestUser(const QString &username, const QString &password, const QString &email)
{
    bool result = userManager->registerUser(username, password, email);
    QVERIFY(result);
}

GameRecord TestUserManager::createTestGameRecord(const QString &opponent, const QString &result)
{
    GameRecord record;
...
{
    bool result = userManager->registerUser("testuser", "testpass", "test@example.com");
    QVERIFY(result);
...
{
    // Register first user
    bool result1 = userManager->registerUser("duplicate", "pass1", "email1@test.com");
    QVERIFY(result1);

    // Try to register same username
    bool result2 = userManager->registerUser("duplicate", "pass2", "email2@test.com");
    QVERIFY(!result2);
}
...
{
    bool result = userManager->registerUser("emailuser", "password", "user@example.com");
    QVERIFY(result);
...
void TestUserManager::testSaveGameHistory()
{
    ...
    QList<GameRecord> savedHistory = userManager->loadGameHistoryFromDatabase("historyuser");
...
void TestUserManager::testLoadGameHistory()
{
    ...
    for (int i = 0; i < 3; i++) {
        GameRecord record = createTestGameRecord(QString("Opponent%1").arg(i), "Won");
        history.append(record);
    }
    ...
    // Load history
    QList<GameRecord> loadedHistory = userManager->loadGameHistoryFromDatabase("loadhistory");
...
void TestUserManager::testLoadAllGameHistory()
{
    ...
    for (int i = 0; i < 7; i++) {
        GameRecord record = createTestGameRecord(QString("AllOpponent%1").arg(i), "Won");
        history.append(record);
    }
    ...
    QList<GameRecord> limitedHistory = userManager->loadGameHistoryFromDatabase("allhistory");
    QCOMPARE(limitedHistory.size(), 5);

    // Load all history (should be 7)
    QList<GameRecord> allHistory = userManager->loadAllGameHistoryFromDatabase("allhistory");
...
void TestUserManager::testGameHistoryLimit()
{
    ...
    for (int i = 0; i < 10; i++) {
        GameRecord record = createTestGameRecord(QString("LimitOpponent%1").arg(i), "Won");
        history.append(record);
    }
    ...
    QList<GameRecord> limitedHistory = userManager->loadGameHistoryFromDatabase("limituser");
    QCOMPARE(limitedHistory.size(), 5);
}
...
void TestUserManager::testGameHistoryOrdering()
{
    ...
    for (int i = 0; i < 3; i++) {
        GameRecord record = createTestGameRecord(QString("OrderOpponent%1").arg(i), "Won");
        record.timestamp = QDateTime::currentDateTime().addSecs(-i * 60); // Different times
        history.append(record);
    }
    ...
    QList<GameRecord> orderedHistory = userManager->loadGameHistoryFromDatabase("orderuser");
...
void TestUserManager::testMovesToString()
{
    ...
    QList<GameRecord> loadedHistory = userManager->loadGameHistoryFromDatabase("moveuser");
...
void TestUserManager::testMovesFromString()
{
    ...
    // Load and verify
    QList<GameRecord> loadedHistory = userManager->loadGameHistoryFromDatabase("stringuser");
...
void TestUserManager::testMoveSerializationRoundTrip()
{
    ...
    // Load and compare
    QList<GameRecord> loadedHistory = userManager->loadGameHistoryFromDatabase("roundtrip");
...
void TestUserManager::testEmptyMoveSerialization()
{
    ...
    QList<GameRecord> loadedHistory = userManager->loadGameHistoryFromDatabase("emptymoves");
