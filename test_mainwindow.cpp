...
void TestMainWindow::testMainWindowInitialization()
{
    ...
    QCOMPARE(mainWindow->windowTitle(), QString("🎮 Tic Tac Toe - Sign In to Play!"));
...
void TestMainWindow::testLoginScreenNavigation()
{
    ...
    if (showRegisterBtn) {
        ...
        QTest::qWait(100);
        QCOMPARE(mainWindow->windowTitle(), QString("📝 Tic Tac Toe - Create Account!"));
    }
}
...
void TestMainWindow::testDifficultySelection()
{
    ...
    QList<QRadioButton*> difficultyBtns = mainWindow->findChildren<QRadioButton*>();
...
void TestMainWindow::testPlayerNamesInput()
{
    ...
    if (nameEdits.size() >= 2) {
        ...
        QList<QPushButton*> buttons = mainWindow->findChildren<QPushButton*>();
        for (QPushButton *btn : buttons) {
            if (btn->text().contains("Start Game") || btn->text().contains("CONFIRM")) {
                confirmBtn = btn;
                break;
            }
        }
...
void TestMainWindow::testCellInteraction()
{
    ...
    for (QPushButton *btn : allButtons) {
        if (btn->text().isEmpty() && btn->isEnabled() &&
            btn->size().width() > 80 && btn->size().height() > 80) {
            gameCell = btn;
            break;
        }
    }
...
void TestMainWindow::testNewGameFunctionality()
{
    ...
    QList<QPushButton*> buttons = mainWindow->findChildren<QPushButton*>();
    for (QPushButton *btn : buttons) {
        if (btn->text().contains("PLAY AGAIN") || btn->text().contains("New Game")) {
            newGameBtn = btn;
            break;
        }
    }
...
void TestMainWindow::testUserProfileDisplay()
{
    ...
    QVERIFY(hasWelcomeLabel || hasStatsLabel); // Should have some profile content
}
...
void TestMainWindow::testButtonAnimations()
{
    ...
    for (QPushButton *btn : buttons) {
        if (btn->isEnabled() && btn->text().isEmpty() &&
            btn->size().width() > 80) {
            gameCell = btn;
            break;
        }
...
void TestMainWindow::testReplayControls()
{
    ...
    for (QPushButton* btn : buttons) {
        if (btn->text().contains("Play")) playBtn = btn;
        else if (btn->text().contains("Pause")) pauseBtn = btn;
        else if (btn->text().contains("Reset")) resetBtn = btn;
    }
